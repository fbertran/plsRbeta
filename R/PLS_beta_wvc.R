PLS_beta_wvc <- function(dataY,dataX,nt=2,dataPredictY=dataX,modele="pls",family=NULL,scaleX=TRUE,scaleY=NULL,keepcoeffs=FALSE,keepstd.coeffs=FALSE,tol_Xi=10^(-12),weights,method="logistic",link=NULL,link.phi=NULL,type="ML",usegpu=FALSE) {
...
if (kk==1) {
#assign("YwotNA", YwotNA, envir=parent.frame(n=sys.nframe()))
tempconstbeta <- betareg::betareg(YwotNA~1,hessian=TRUE,model=TRUE,link=link,link.phi=link.phi,type=type,phi=FALSE)
res$Coeffsmodel_vals <- rbind(summary(tempconstbeta)$coefficients$mean,matrix(rep(NA,4*nt),ncol=4))
rm(tempconstbeta)
tt<-res$tt
#assign("YwotNA", YwotNA, envir=parent.frame(n=sys.nframe()))
#assign("tt", tt, envir=parent.frame(n=sys.nframe()))
tempregbeta <- betareg::betareg(YwotNA~tt,hessian=TRUE,model=TRUE,link=link,link.phi=link.phi,type=type,phi=FALSE)
rm(tt)
...
res$predphis <- predict(tempregbeta,type="precision")
if(!usegpu){
tempCoeffs <- res$wwetoile%*%res$CoeffC*attr(res$RepY,"scaled:scale")/attr(res$ExpliX,"scaled:scale")
tempConstante <- attr(res$RepY,"scaled:center")-sum(tempCoeffs*attr(res$ExpliX,"scaled:center"))+attr(res$RepY,"scaled:scale")*res$Std.Coeffs[1]
res$Coeffs <- rbind(tempConstante,tempCoeffs)
...
if (!keepcoeffs) {
if (!keepstd.coeffs) {return(list(valsPredict=res$listValsPredictY, valsPredictPhis=res$listValsPhisPredictY))} else {return(list(valsPredict=res$listValsPredictY, valsPredictPhis=res$listValsPhisPredictY, std.coeffs=res$Std.Coeffs))}}
else {
if (!keepstd.coeffs) {return(list(valsPredict=res$listValsPredictY, valsPredictPhis=res$listValsPhisPredictY, coeffs=res$Coeffs))} else {return(list(valsPredict=res$listValsPredictY, coeffs=res$Coeffs, valsPredictPhis=res$listValsPhisPredictY, std.coeffs=res$Std.Coeffs))}
}
} else {
if (!keepcoeffs) {
